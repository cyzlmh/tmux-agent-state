...
tmux-agent-state/
  ...
  panel/
    agent-panel.tmux   tmux bootstrap (interpolates #{agent_panel} + window-status formats)
  ...
  (claude/, codex/     future: hook-config adapters, same protocol)
```

Consumers: the status-bar segment in `panel/` (tmux-native, aligned with
...
agent adapter (the panel runs without pi, but then has nothing to track).
...
tmux set -g status-right '#{agent_panel} | %H:%M'
tmux run-shell ~/tmux-agent-state/panel/agent-panel.tmux

# 2. pi adapter: symlink the extension, then /reload inside pi
ln -s ~/tmux-agent-state/pi/agent-state.ts ~/.pi/agent/extensions/agent-state.ts
```
...
tmux run-shell ~/tmux-agent-state/panel/agent-panel.tmux   # once per server
```
...
`@agent-panel-scope` (session|window), `@agent-panel-color-*`.
...
(`TMUX_PANEL_COLORIZE` overrides the path; empty string disables), and the
...
| adapter present, ts stale (>45s) | stale (red)  |

`state` comes from adapter events only (input/agent_start -> busy,
agent_settled -> waiting); `detail=asking` is reliable too — the pi
`question` tool writes waiting/asking while it blocks on the user (see
`pi/README.md`). Other `detail` values are display hints only.

## Status

| adapter | form | status |
| ------- | ---- | ------ |
| pi      | TS extension (`pi/agent-state.ts`) | done, e2e verified |
| claude  | `~/.claude/settings.json` hooks   | not started |
| codex   | `~/.codex/hooks.json` hooks        | not started |
...
```sh
cd tmux-agent-state
# type-check (needs the pi package + @types/node resolvable; see pi/README.md)
